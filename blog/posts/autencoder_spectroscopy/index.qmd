...
Traditional smoothing techniques are the equivalent of using a bulldozer to dust off a delicate vase. Sure, you'll remove the dirt, but you might also remove, well, everything else. One use case of autoencoders is to do this in a better way. 
...
Now that we've seen that linear autoencoders are just PCA in disguise, let's talk about why we still bother with neural networks.
...
Here's where the VAE truly shines: it doesn't just learn the manifold, it learns a *probabilistic* manifold with a well-behaved coordinate system. The VAE loss function -- the negative evidence lower bound (ELBO), which we minimize -- has two terms:
