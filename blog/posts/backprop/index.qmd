...
description: <i>Efficient long-distance error propagation</i>
image: ""
sidebar: false
categories:
  - machine-learning
...
When we think of the tiny neural network in the widget above one might think of many different ways for optimizing the weights (line strengths) of this model.
...
Using a strategy like a grid search (in which you loop over a range of possible weight values for all weights) will also only work for small models (think of the $100^4$ combinations you would have to try for just 100 trial values of 4 weights). 
...
To create this plot, we keep two weights fixed, vary two others and then analyze what the loss looks like.
...
The problem is that we need to perform _many evaluations_ of the loss to make it work (one per weight, which can be a lot for current frontier models). In addition, we add up errors because $h$ will be different from $0$ (truncation error) and because we have to work with machine precision and hence add rounding errors.

::: {.callout-note}

If we compute numerical gradients, we have two main sources of error. One stems from the fact that $h$ in the equation above is not exactly 0. This is known as truncation error. On the other hand, the finite difference equation leads to numerical problems (rounding errors) as two almost identical numbers are subtracted and then divided by a very small number. 
...
Obviously, we could save many evaluations if we could write down the derivatives for a given function. 
However, for our neural networks we cannot do this by hand. 

The question is thus how we _efficiently_ compute the gradient of a function such as a neural network.
...
The rate of change in relative positions is given by terms like $\frac{\mathrm{d}x}{\mathrm{d}y}$, which gives us the change in relative position of bicycle and car. If we now aim to compute the rate of change of relative position of car to the walking man, $\frac{\mathrm{d}x}{\mathrm{d}z}$, we find 

$$
\frac{\mathrm{d}x}{\mathrm{d}z} = \frac{\mathrm{d}x}{\mathrm{d}y} \frac{\mathrm{d}y}{\mathrm{d}z} = \underbrace{2}_{\text{car twice as fast as bicycle}} \cdot \underbrace{4}_{\text{bicycle is four times as fast as walking man}} = 8 
$$
...
In neural networks, we nest functions. That is, we will end up differentiating compound expressions of the form 
...
Phew! That was a lot of copying and pasting and quite error prone. And it might be quite costly to just directly evaluate such an expression (we might end up with an exponentially large expression, "expression swell").

There must be a better way.

### Making it efficient with caching 

One thing that we can observe is that we need to do the same computation several times. For instance, $wx +b$ is evaluated two times. We can trade off space and time complexity by caching this using an intermediate variable. 

If we do this systematically, we can very efficiently compute gradients -- in a form that is symmetric to the computation of the function itself (and thus with basically the same cost). 
...
It can be convenient to introduce the following intermediate variable
...
$$
\frac{\partial f}{\partial p} = z \quad \frac{\partial f}{\partial z} = p
$$
...
$$
\frac{\partial f(p,z)}{\partial x} = \frac{\partial f(p, z)}{\partial p}  \frac{\partial p(x,y)}{\partial x} 
$$
...
$$
\begin{align}
\frac{\mathrm{d}f}{\mathrm{d} t_{n-1}} &= \frac{\mathrm{d}f}{\mathrm{d}t_{n}} \frac{\mathrm{d}t_{n}}{\mathrm{d}t_{n-1}} \\
\frac{\mathrm{d}f}{\mathrm{d} t_{n-2}} &= \frac{\mathrm{d}f}{\mathrm{d}t_{n}} \frac{\mathrm{d}t_{n}}{\mathrm{d}t_{n-1}} \frac{\mathrm{d}t_{n-1}}{\mathrm{d}t_{n-2}} \\
\frac{\mathrm{d}f}{\mathrm{d} t_{n-3}} &= \frac{\mathrm{d}f}{\mathrm{d}t_{n}} \frac{\mathrm{d}t_{n}}{\mathrm{d}t_{n-1}} \frac{\mathrm{d}t_{n-1}}{\mathrm{d}t_{n-2}} \frac{\mathrm{d}t_{n-2}}{\mathrm{d}t_{n-3}} \\
\frac{\mathrm{d}f}{\mathrm{d} t_i} &= \frac{\mathrm{d}f}{\mathrm{d}t_{n}} \frac{\mathrm{d}t_{n}}{\mathrm{d}t_{n-1}} \frac{\mathrm{d}t_{n-1}}{\mathrm{d}t_{n-2}} \ldots \frac{\mathrm{d}t_{i+1}}{\mathrm{d}t_{i}}
\end{align} 
$$
...
\frac{\partial t_4}{\partial t_3} &= \exp(t_3) \\
...
$$
\begin{align}
\frac{\partial t_8}{\partial t_6} &= \frac{\partial t_8}{\partial t_7} \frac{\partial t_7}{\partial t_6} = 2 t_7 \cdot 1 = 2(t_6 -t) \\
\frac{\partial t_8}{\partial t_5} &= \frac{\partial t_8}{\partial t_6} \frac{\partial t_6}{\partial t_5} = 2(t_6 -t) \cdot  \left(-\frac{1}{t_5^2} \right) =  -2/t_5^2 (t_6 -t) \\
\frac{\partial t_8}{\partial t_4} &= \frac{\partial t_8}{\partial t_5} \frac{\partial t_5}{\partial t_4} = -2/t_5^2 (t_6 -t) \cdot 1 = -2/t_5^2 (t_6 -t) \\
\frac{\partial t_8}{\partial t_3} &= \frac{\partial t_8}{\partial t_4} \frac{\partial t_4}{\partial t_3} = -2/t_5^2 (t_6 -y) \cdot \exp(t_3) = -2/t_5^2 (t_6 -t) \cdot \exp(t_3) \\
\frac{\partial t_8}{\partial t_2} &= \frac{\partial t_8}{\partial t_3} \frac{\partial t_3}{\partial t_2} = -2/t_5^2 (t_6 -t) \cdot \exp(t_3) \cdot -1 = 2/t_5^2 (t_6 -t) \cdot \exp(t_3) \\
\frac{\partial t_8}{\partial t_1} &= \frac{\partial t_8}{\partial t_2} \frac{\partial t_2}{\partial t_1} =  2/t_5^2 (t_6 -t) \cdot \exp(t_3) \\
\frac{\partial t_8}{\partial w} &= \frac{\partial t_8}{\partial t_1} \frac{\partial t_1}{\partial w} = 2/t_5^2 (t_6 -t) \cdot \exp(t_3) \cdot x
\end{align}
$$

In practice, we would use autodifferentiation using a data structure as follows to keep track of the computation graph.
...
1. [Andrej Karpathy "Hacker's guide to Neural Networks"](https://karpathy.github.io/neuralnets/) inspired the comparison between random search and gradient descent. The same ideas are used in the [cs231n lecture notes](https://cs231n.github.io/optimization-1/) since he taught this class. The chain rule example is taken from the [cs231n lecture notes](https://cs231n.github.io/optimization-2/)

2. [Andrej Karpathy recorded a lecture in which he builds an autodiff system from scratch](https://www.youtube.com/watch?v=PaCmpygFfXo) and it inspired many parts of the notebooks, some parts (the `Value` class) are taken from his lecture.
...
4. [Mark Saroufim "Automatic Differentiation Step by Step"](https://marksaroufim.medium.com/automatic-differentiation-step-by-step-24240f97a6e6) has an intuitive explanation of dual numbers and has a good resource section, including [this talk](https://www.youtube.com/watch?v=Rs0uRQJdIcg&list=WL&index=8&t=149s)
...
7. [Brandon Rohrer](https://e2eml.school/how_backpropagation_works) has a very intuitive explanation of the chain rule in terms of the shower rate (similar to the bicycle/car/man example above).

8. [Deep Learning Systems Lecture at CMU](https://dlsyscourse.org/lectures/) has detailed slides on the algorithmic details behind autodiff.
...
In the context of training neural networks, backpropagation was popularized in a beautiful paper by [David E. Rumelhart et al.](https://www.nature.com/articles/323533a0) It is beautiful and you should read it. 
...
- Numerical differentiation involves computing a term like $\frac{\partial f}{\partial x_i} \approx \frac{f(x+h) - f(x)}{h}$ for a small $h$. While this might be relatively easy to implement, it requires $\mathcal{O}(n)$ evaluations for $n$ gradients, and can be numerically unstable (dividing by small number, subtracting two numbers of almost the same value).
- Symbolic differentiation can be performed with systems like Maple, Sympy, or Mathematica. This gives us _expressions_ for the derivatives, which might grow exponentially large (in blind application).
...
Now, keep in mind the Taylor series of a function $f(x)$:
...
note that, per definition, all terms with $\epsilon^2$ or higher powers will vanish. Therefore, we will be left with 
...
This means that we directly compute f(x) and the derivative (scaled by $\dot{v}$). Thus, we can simultaneously compute the values of functions and derivatives. A naive implementation might look as follows
...
- [jax-md](https://github.com/jax-md/jax-md): Which allows one to differentiate through full MD simulations, to do things like [the design of kinetic pathways](https://www.pnas.org/doi/abs/10.1073/pnas.2024083118)
